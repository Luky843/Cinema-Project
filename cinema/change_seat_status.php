...
function whoSeatUse($seat_num,$movieID)
{
	$db_cr = get_DB_config();
	$conn = new mysqli($db_cr[0],$db_cr[1],$db_cr[2],$db_cr[3]);
	$sql = "select * from seats where num=".$seat_num." and movie = ".$movieID.";";
	$result = $conn->query($sql);
	if($result->num_rows > 0)
	{
		if($rws = $result->fetch_assoc())
...
function reserveSeat($movieID,$seat_num,$user_id)
{
	$db_cr = get_DB_config();
	$conn = new mysqli($db_cr[0],$db_cr[1],$db_cr[2],$db_cr[3]);
	$sql = "update seats set isReseved=1, usr=".$user_id." where num=".$seat_num." and movie=".$movieID.";";
	$conn->query($sql);
	$conn->close();
}
...
function unReserve($movieID,$seat_num,$user_id)
{
	$db_cr = get_DB_config();
	$conn = new mysqli($db_cr[0],$db_cr[1],$db_cr[2],$db_cr[3]);
	$sql = "update seats set isReseved=0, usr=NULL where num=".$seat_num." and movie=".$movieID." and usr=".$user_id.";";
	$conn->query($sql);
	$conn->close();
}
